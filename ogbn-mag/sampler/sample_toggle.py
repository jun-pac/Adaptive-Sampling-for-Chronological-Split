...
def python_sample(rowptr: torch.Tensor, col: torch.Tensor, idx: torch.Tensor,num_ppr_neighbors: int,
                num_atr_neighbors: int, num_ins_neighbors: int, num_fld_neighbors: int, relation_ptr: torch.Tensor=None
    ...
    for i in range(idx.numel()):
        ...
        ppr_count=int(relation_ptr[4*n+1]-relation_ptr[4*n])
        atr_count=int(relation_ptr[4*n+2]-relation_ptr[4*n+1])
        ins_count=int(relation_ptr[4*n+3]-relation_ptr[4*n+2])
        fld_count=int(relation_ptr[4*n+4]-relation_ptr[4*n+3])
        perm=set()
        ...
        # Sample fld
        if(fld_count <= num_fld_neighbors):
            for j in range(fld_count):
                perm.add(ppr_count+atr_count+ins_count+j)
        else :
            for j in range(fld_count-num_fld_neighbors, fld_count):
                temp=np.random.randint(0,j)
                if (not temp in perm):
                    perm.add(ppr_count+atr_count+ins_count+temp)
                else:
                    perm.add(ppr_count+atr_count+ins_count+j)
...
def sample_adj(src: SparseTensor, subset: torch.Tensor, num_ppr_neighbors: int,
                num_atr_neighbors: int, num_ins_neighbors: int, num_fld_neighbors: int,
    ...
    rowptr, col, n_id, e_id = python_sample(
        rowptr, col, subset, num_ppr_neighbors, num_atr_neighbors, num_ins_neighbors, num_fld_neighbors, relation_ptr)
...
class NeighborSampler(torch.utils.data.DataLoader):
    ...
    def sample(self, batch):
        ...
        for i, size in enumerate(self.sizes):
            num_ppr, num_atr, num_ins, num_fld = size
            ...
            cur_adj_t, n_id = sample_adj(cur_adj_t, n_id, num_ppr, num_atr, num_ins, num_fld, relation_ptr=cur_relation_ptr)
            ...
            adjs.append(Adj(cur_adj_t, e_id, size))
