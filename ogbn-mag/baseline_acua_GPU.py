...
class MAG240M(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        ...
        pyg_dataset = PygNodePropPredDataset(name='ogbn-mag', root=self.data_dir)
        dataset=pyg_dataset[0]
        split_index = pyg_dataset.get_idx_split()

        self.train_idx = split_index['train']['paper']
        self.valid_idx = split_index['valid']['paper']
        self.test_idx = split_index['test']['paper']
        self.test_challenge_idx = split_index['test']['paper']
        ...
        print(f"Initial size - train : {len(self.train_idx)}, valid : {len(self.valid_idx)}, test : {len(self.test_idx)}")
        ...
        self.train_label=dataset.y_dict['paper'][self.train_idx]
        self.valid_label=dataset.y_dict['paper'][self.valid_idx]
        ...
        t1=time.time()
        x = np.memmap(f'{self.data_dir}/ogbn_mag/full_feat.npy', dtype=np.float16,
        ...
        self.num_papers=dataset.num_nodes_dict['paper']
        self.paper_year=dataset.node_year['paper'] # load years to memory!
    ...
    def train_dataloader(self):
        if args.link=='toggle':
            return NS_toggle(self.mono_adj_t, self.adj_t, node_idx = self.train_idx if args.debug==False else self.train_idx[:10000],
    ...
    def convert_batch_train(self, batch_size, n_id, adjs):
        x = self.x[n_id]
        append_label = np.zeros((len(n_id), self.num_classes),dtype=np.float16)
...
class RGNN(LightningModule):
    ...
    def training_step(self, batch, batch_idx: int):
        ...
        #print(f"y_hat.shape : {y_hat.shape}, batch.y.shape : {batch.y.shape}")

        train_loss = F.cross_entropy(y_hat, batch.y.squeeze(dim=-1))
        ...
        if((args.debug and batch_idx%10==0) or batch_idx%100==0):
            print(f"{self.current_epoch} epoch ; {name[1:]} | train_acc : {tmp_acc:.5f} | time : {time.time()-t0:.2f} | batch : {batch_idx}/{len(datamodule.train_idx)//args.batch_size}")
            f_log.write(f"{self.current_epoch} epoch ; {name[1:]} | train_acc : {tmp_acc:.5f} | time : {time.time()-t0:.2f} | batch : {batch_idx}/{len(datamodule.train_idx)//args.batch_size}\n")
            f_log.flush()
        return train_loss
    ...
    def test_step(self, batch, batch_idx: int):
        ...
        #    print(f"{name[1:]} | test_acc : {tmp_acc:.5f} | time : {time.time()-t0:.2f} | batch : {batch_idx}/{len(datamodule.test_idx)//datamodule.batch_size}")
        #    f_log.write(f"{name[1:]} | test_acc : {tmp_acc:.5f} | time : {time.time()-t0:.2f} | batch : {batch_idx}/{len(datamodule.test_idx)//datamodule.batch_size}\n")
        #    f_log.flush()
...
if __name__ == '__main__':
    ...
    if not args.evaluate:
        ...
        if args.ckpt != None:
            ...
            model = RGNN(args.model, datamodule.num_features+datamodule.num_classes+args.bit,
                    datamodule.num_classes, args.hidden_channels,
                    datamodule.num_relations, num_layers=len(sizes),
                    dropout=args.dropout)
        if args.debug:
            print(f"Input size : {datamodule.num_features+datamodule.num_classes+args.bit}")
    ...
    # Evaluate
        ...
        f_log.write("Original : [[40,10,0,10],[20,10,5,5]]\n")
        f_log.flush()
        datamodule.sizes = [[40,10,0,10],[20,10,5,5]] 
        trainer.test(model=model, datamodule=datamodule)

        f_log.write("X3 : [[120,30,0,30],[60,30,15,15]]\n")
        f_log.flush()
        datamodule.sizes = [[120,30,0,30],[60,30,15,15]] 
        trainer.test(model=model, datamodule=datamodule)

        f_log.write("X4 : [[160,40,0,40],[80,40,20,20]]\n")
        f_log.flush()
        datamodule.sizes = [[160,40,0,40],[80,40,20,20]] 
        trainer.test(model=model, datamodule=datamodule)

        f_log.write("X5 : [[200,50,0,50],[100,60,25,25]]\n")
        f_log.flush()
        datamodule.sizes = [[200,50,0,50],[100,60,25,25]]  # (Almost) no sampling...
        trainer.test(model=model, datamodule=datamodule)
